...
function getEmojis() {
  try {
    ...
    logger.error('[EmojiReplacer] Error loading emojis.json:', e);
  }
  return {};
}
...
function replaceEmojis(text) {
  ...
  let result = text.replace(/<(a)?:(\w{2,32}):(\d{17,20})>/g, (match, animated, name) => {
...
module.exports = { replaceEmojis, replaceEmojisInObject };

logger.info('[EmojiReplacer] Global Discord Emoji translation hook initialized.');

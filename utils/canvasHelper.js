...
try {
  ...
  logger.error('[CanvasHelper] Failed to register font:', err.message);
}
...
function resolveImagePath(url) {
  ...
  return path.isAbsolute(url) ? url : path.join(__dirname, '..', url);
}
...
async function generateShowcaseCard(avatarUrl, bannerUrl, username = 'Discord User') {
  ...
  if (bannerUrl) {
    try {
      ...
      logger.error('[CanvasHelper] Failed to load banner image, falling back to gradient:', err.message);
  ...
  if (avatarUrl) {
    try {
      ...
      logger.error('[CanvasHelper] Failed to load avatar image, drawing default:', err.message);
  ...
  for (const file of badgeFiles) {
    try {
      const badgeImg = await loadImage(resolveImagePath(file));
      ctx.drawImage(badgeImg, currentBadgeX, currentBadgeY, badgeSize, badgeSize);
      currentBadgeX += badgeSize + badgeSpacing;
    } catch (err) {
      logger.error(`[CanvasHelper] Failed to load badge ${file}:`, err.message);
    }
  }
...
async function generateNitroCard(nitroData) {
  const scale = 2;
  const W = 1200,
    H = 500;
  ...
  let glowX = W / 2;
  const glowY = badgeY + badgeSize / 2;
  if (tierIndex !== -1) {
  ...
  for (let i = 0; i < allBadges.length; i++) {
    ...
    try {
      const badgeImg = await loadImage(resolveImagePath(`assets/badges/nitro/${name}.png`));
      ...
      // Silently fail for individual badge loading errors
    }
  }
  ...
  ctx.textAlign = 'left';
  const monthsLabel =
    nitroData.totalMonths !== null ? `${nitroData.totalMonths} / ${nitroData.progressEnd} أشهر` : `✦ نيترو نشط`;
  ...
  if (nitroData.timeRemainingMs !== null) {
    ...
    daysText = `متبقي大约 ${nitroData.daysRemaining} يوم للشارة القادمة`;
    ctx.fillStyle = '#ff73fa';
...
async function generateBoostCard(boostData) {
  const scale = 2;
  const W = 1200,
    H = 500;
  const canvas = createCanvas(W * scale, H * scale);
  ...
  for (let t = 1; t <= badgeCount; t++) {
    ...
    try {
      const badgeImg = await loadImage(resolveImagePath(`assets/badges/boost/discordboost${t}.svg`));
      ...
      // Silently fail for boost badges - not critical
    }
  }
...
async function generateTempVCPanel() {
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    try {
      ...
      logger.error('Failed to load icon:', buttons[i].icon);
    }

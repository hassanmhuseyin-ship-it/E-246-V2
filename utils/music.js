...
function setupMusic(client) {
  ...
  if (nodes.length === 0) {
    logger.warn(
      '[Music] No Lavalink nodes found in .env (LAVALINK_NODES). Music system will be disabled or fail to connect.'
    );
  }
  ...
  client.manager.nodeManager.on('connect', (node) => logger.info(`[Music] Node: ${node.id} connected.`));
  client.manager.nodeManager.on('reconnecting', (node) => logger.info(`[Music] Node: ${node.id} reconnecting.`));
  client.manager.nodeManager.on('disconnect', (node) => logger.info(`[Music] Node: ${node.id} disconnected.`));
  client.manager.nodeManager.on('error', (node, err) =>
    logger.error(`[Music] Node: ${node.id} error: ${err.message}`)
  );
        ...
        channel
          .send({
            embeds: [
              new EmbedBuilder()
                .setColor(0xed4245)
                .setDescription(`{emoji:circlex} **انقطع الاتصال وتم الخروج من <#${oldChannel}>**`)
            ]
          })
          .catch(() => {});
      }
  ...
  client.manager.on('trackStart', async (player, track) => {
    ...
    const trackEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '{emoji:clock} المدة',
          value: track.info.isStream ? `\`🔴 بث مباشر\`` : `\`${formatDuration(track.info.duration)}\``,
          inline: true
        }
      );
  ...
  client.manager.on('queueEnd', async (player, track) => {
    ...
    const endMsg = await client.channels.cache
      .get(player.textChannelId)
    ...
    if (!player.playing && !twentyFourSeven) {
      setTimeout(async () => {
        if (!player.playing && player.connected) {
          const disconnEmbed = new EmbedBuilder().setColor(0xed4245).setDescription('تم قطع الاتصال بسبب الخمول.');
          const disconnMsg = await client.channels.cache
            .get(player.textChannelId)
            ?.send({ embeds: [disconnEmbed] })
            .catch(() => null);
...
function createController(guildId, player) {
  ...
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setStyle(ButtonStyle.Secondary).setCustomId(`controller:${guildId}:Stop`).setEmoji(stopEmoji),
    new ButtonBuilder()
    ...
    new ButtonBuilder().setStyle(ButtonStyle.Secondary).setCustomId(`controller:${guildId}:Next`).setEmoji(nextEmoji),
    new ButtonBuilder()
      .setStyle(ButtonStyle.Secondary)
      .setCustomId(`controller:${guildId}:Loop`)
      .setEmoji(
        player.repeatMode === 'track' ? loopTrackEmoji : player.repeatMode === 'queue' ? loopQueueEmoji : loopNoneEmoji
      )
  );
}

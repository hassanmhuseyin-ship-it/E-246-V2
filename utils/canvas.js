...
async function createRankCard(user, textXp, textLevel, voiceTime, voiceLevel) {
  ...
  try {
    ...
    logger.error('Canvas avatar error:', e.message);
  }
  ...
  try {
    ...
    logger.error('[canvas] Failed to load text_icon.svg:', e.message);
  }
  ...
  try {
    ...
    logger.error('[canvas] Failed to load voice_icon.svg:', e.message);
  }
...
async function createLeaderboardCanvas(guild, topUsers, requestingUserRank = null) {
  ...
  try {
    ...
    logger.error('[canvas] Failed to load leaderboard icons:', e.message);
  }
  ...
  async function drawUserCard(userRecord, x, y, rank) {
    ...
    if (rank === 1) {
      ...
      badgeBorder = 'rgba(192, 192, 192, 0.5)';
      badgeText = '#C0C0C0';
    } else if (rank === 3) {

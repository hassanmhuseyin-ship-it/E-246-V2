    ...
    try {
      ...
      if (existing) {
        ...
        return interaction.reply({
          embeds: [success('تعطيل الفاصل التلقائي', `تم تعطيل الفاصل التلقائي في ${channel}`)]
        });
      } else {
        db.addAutomation(interaction.guildId, channel.id, 'autoline', 'enabled');
        return interaction.reply({
          embeds: [success('تفعيل الفاصل التلقائي', `تم تفعيل الفاصل التلقائي في ${channel}`)]
        });
      }
    } catch (err) {
      logger.error('[Command Error - autoline.js]:', err);

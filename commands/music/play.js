...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('song').setDescription('اسم الأغنية أو الرابط').setRequired(true).setAutocomplete(true)
    ),
    ...
    try {
      ...
      if (!query || !query.trim()) {
        return interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xed4245)
              .setDescription(`{emoji:circlex} **` + 'Please provide a valid song name or URL.' + `**`)
          ]
        });
      }
      ...
      if (res.loadType === 'error') {
        ...
        return interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xed4245)
              .setDescription(`{emoji:circlex} **` + 'There was an error while searching for that song.' + `**`)
          ]
        });
      }
      ...
      if (res.loadType === 'empty') {
        ...
        return interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xed4245)
              .setDescription(`{emoji:circlex} **` + 'No results were found for that query.' + `**`)
          ]
        });
      }
      ...
      if (res.loadType === 'track' || res.loadType === 'search') {
        ...
        const addEmbed = new EmbedBuilder()
          ...
          .setThumbnail(
            track.info.artworkUrl ?? `https://img.youtube.com/vi/${track.info.identifier}/maxresdefault.jpg`
          );
      ...
      logger.error('[Command Error - play.js]:', err);

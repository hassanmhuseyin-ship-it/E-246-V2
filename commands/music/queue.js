...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const cleanTitle = (t) => escapeMarkdown(t.info.title).replace(/\[/g, '').replace(/\]/g, '');
      ...
      const queueDuration = tracks.reduce((acc, t) => acc + (t.info.isStream ? 0 : t.info.duration), 0);

      const mapping = tracks.map((t, i) => `\`${i + 1}\` [${cleanTitle(t)}](${t.info.uri}) [${t.requester}]`);
      ...
      logger.error('[Command Error - queue.js]:', err);

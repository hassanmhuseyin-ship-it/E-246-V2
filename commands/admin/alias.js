...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) => sub.setName('list').setDescription('عرض قائمة الاختصارات الحالية')),
  ...
  async execute(interaction) {
    try {
      ...
      if (sub === 'add') {
        ...
        if (!baseCmd) {
          return interaction.reply({
            embeds: [error(`الأمر الأساسي \`${baseCommandName}\` غير موجود.`)],
            flags: ['Ephemeral']
          });
        }
        ...
        return interaction.reply({
          embeds: [
            success(`تمت إضافة الاختصار بنجاح!`, `يمكنك الآن استخدام \`#${shortcut}\` لتنفيذ أمر \`${command}\``)
          ]
        });
      }
      ...
      logger.error('[Command Error - alias.js]:', err);

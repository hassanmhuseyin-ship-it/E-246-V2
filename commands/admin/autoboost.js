    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addChannelOption((o) =>
          o
            .setName('channel')
            .setDescription('روم البوست')
            .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement)
            .setRequired(true)
        )
        .addStringOption((o) => o.setName('message').setDescription('رسالة الشكر').setRequired(true))
    )
    .addSubcommand((sub) => sub.setName('disable').setDescription('تعطيل رسالة البوست'))
    ...
    try {
      ...
      if (sub === 'setup') {
        ...
        return interaction.reply({
          embeds: [
            success(`تم تفعيل رسالة البوست التلقائية بنجاح\n\n**الروم** <#${channel.id}>\n**الرسالة**\n${message}`)
          ]
        });
      }
      ...
      logger.error('[Command Error - autoboost.js]:', err);

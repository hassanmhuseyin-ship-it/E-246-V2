    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addChannelOption((opt) =>
          opt.setName('master_channel').setDescription('الروم الصوتي الأساسي').setRequired(true)
        )
        .addChannelOption((opt) => opt.setName('category').setDescription('قسم الرومات الجديدة').setRequired(true))
    )
    .addSubcommand((sub) =>
      sub
        .setName('panel')
      ...
      if (sub === 'setup') {
        ...
        if (masterChannel.type !== 2) {
          return interaction.reply({
            embeds: [error('يرجى تحديد قناة صوتية صحيحة كغرفة أساسية')],
            flags: ['Ephemeral']
          });
        }
        if (category.type !== 4) {
        ...
        const embed = success(
          `تم إعداد الغرف الصوتية المؤقتة بنجاح\n\nالغرفة الأساسية <#${masterChannel.id}>\nالقسم **${category.name}**`
        );
        ...
        return interaction.reply({
          embeds: [success(`تم إرسال لوحة التحكم بنجاح في روم <#${channel.id}>`)],
          flags: ['Ephemeral']
        });
      }
    } catch (err) {
      logger.error('[Command Error - tempvoice.js]:', err);

...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        .setName('channel')
        .setDescription('تحديد روم السجلات')
        .addChannelOption((o) =>
          o
            .setName('channel')
    ...
    .addSubcommand((s) =>
      s
        .setName('toggle')
        .setDescription('تبديل نوع السجل')
        .addStringOption((o) =>
          o
            .setName('type')
            .setDescription('نوع السجل')
            .setRequired(true)
            .addChoices(...logTypes.map((t) => ({ name: t.replace(/_/g, ' '), value: t })))
        )
    )
    ...
    try {
      ...
      if (sub === 'toggle') {
        ...
        if (!mainLogChannel && !settings[column]) {
          return interaction.reply({
            embeds: [error('لم يتم تحديد روم السجلات', 'حدد روم السجلات أولاً باستخدام `/logs channel`')],
            flags: ['Ephemeral']
          });
        }
        ...
        db.db
          .prepare(`UPDATE log_settings SET ${column} = ? WHERE guildId = ?`)
          .run(current ? null : mainLogChannel, interaction.guildId);
        return interaction.reply({
          embeds: [
            success(
              locale.get('moderation.logToggled', {
                status: current ? 'تعطيل' : 'تفعيل',
      ...
      if (sub === 'show') {
        ...
        const rows = logTypes
          .map(
            (t) =>
              `${settings[logColumns[t]] ? '{emoji:circlecheck}' : '{emoji:circlex}'} \`${t.replace(/_/g, ' ')}\`${settings[logColumns[t]] ? ` → <#${settings[logColumns[t]]}>` : ''}`
          )
          .join('\n');
      ...
      logger.error('[Command Error - logs.js]:', err);

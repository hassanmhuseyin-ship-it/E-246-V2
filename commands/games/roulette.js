...
module.exports = {
  subcommand: new SlashCommandSubcommandBuilder().setName('roulette').setDescription('لعبة الروليت الروسية'),
  ...
  async execute(interaction) {
    ...
    collector.on('end', async () => {
      if (players.length < 2) {
        ...
        return interaction.channel.send(
          `${emojis.circlex} تم إلغاء اللعبة لعدم اكتمال العدد يجب أن يكون هناك لاعبان على الأقل`
        );
      }
      ...
      await interaction.channel.send(
        `${emojis.confetti} بدأنا اللعبة بـ ${players.length} لاعبين جاري تدوير العجلة ${emojis.clock}`
      );
    ...
    async function playRound(channel) {
      ...
      try {
        ...
        logger.error('[Roulette] Error generating GIF:', e.message);
      ...
      const actionEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `يا <@${winner.id}> اختار شخص تطرده برا اللعبة من القائمة أو تقدر تنسحب بنفسك وتعطي فرصة للباقين لديك 30 ثانية للاختيار`
        )
        .setColor(0xff0000);

      const cleanTrashEmoji = emojis.trash
        ? emojis.trash.match(/:(\d+)>/)
          ? emojis.trash.match(/:(\d+)>/)[1]
          : emojis.trash
        : '🗑️';
      ...
      const roundCollector = actionMsg.createMessageComponentCollector({ filter: () => true, time: 30000 });
      ...
      roundCollector.on('end', async (collected, reason) => {
        if (reason !== 'acted') {
          await actionMsg.edit({
            content: `${emojis.alerttriangle} انتهى الوقت <@${winner.id}> لم يتخذ قراراً وتم استبعاده عشوائياً`,
            embeds: [],
            components: []
          });
          players = players.filter((p) => p.id !== winner.id);

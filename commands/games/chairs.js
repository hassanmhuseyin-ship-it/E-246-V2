...
module.exports = {
  subcommand: new SlashCommandSubcommandBuilder().setName('chairs').setDescription('لعبة الكراسي الموسيقية'),
  ...
  async execute(interaction) {
    try {
      ...
      try {
        ...
        logger.error('[chairs] Failed to load emojis.json:', e.message);
      }
      ...
      const playRound = async (channel) => {
        ...
        const roundCollector = actionMsg.createMessageComponentCollector({
      ...
      logger.error('[chairs.js] Game error:', err);
      activeGames.delete(interaction.channelId);

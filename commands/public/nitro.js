...
const { formatExactTime, getMsSince } = require('../../utils/timeFormatters');
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) => option.setName('target').setDescription('العضو لمعلومات النيترو')),
  async getNitroPayload(target, userToken) {
    try {
      ...
      if (!nitroData.hasNitro) {
        return {
          embeds: [
            new EmbedBuilder().setColor(0xed4245).setDescription(`**${target.username}** ليس لديه اشتراك نيترو نشط`)
          ]
        };
      }
      ...
      const currentBadgeName =
        nitroData.currentTier === 'none'
          ? 'Normal'
          : nitroData.currentTier.charAt(0).toUpperCase() + nitroData.currentTier.slice(1);
      const currentBadgeMonths = nitroData.progressStart || 0;
      const currentEmojiStr =
        emojis[
          `nitro_${nitroData.currentBadge === 'none' || !nitroData.currentBadge ? 'normal' : nitroData.currentBadge}`
        ] || '';
      ...
      embed.addFields({
        name: 'Current Level',
        value:
          `${currentEmojiStr} **${currentBadgeName}** ${currentEmojiStr} (${currentBadgeMonths} Month${currentBadgeMonths !== 1 ? 's' : ''})\n` +
      ...
      if (nitroData.nextTier) {
        ...
        embed.addFields({
          name: 'Next Level',
          value:
            `${nextEmojiStr} **${nextBadgeName}** ${nextEmojiStr} (${nextBadgeMonths} Month${nextBadgeMonths !== 1 ? 's' : ''})\n` +
      ...
      logger.error('Failed to fetch selfbot nitro stats or generate components', e);
      throw e;
    }
  },
  async execute(interaction) {
    ...
    if (!userToken) return interaction.editReply({ content: 'توكن حساب الفحص مطلوب' });
    try {

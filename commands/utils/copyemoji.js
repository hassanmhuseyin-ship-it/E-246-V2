...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('emojis').setDescription('الايموجيات للاستخراج').setRequired(true)),
  ...
  async execute(interaction) {
    ...
    if (foundEmojis.length === 0) {
      return interaction.editReply(
        `${emojis.circlex} لم يتم العثور على أي إيموجي مخصص في النص تأكد من وضع إيموجيات سيرفرات مخصصة`
      );
    }

    try {
      const uniqueEmojis = foundEmojis.filter((e, index, self) => index === self.findIndex((t) => t.id === e.id));
      ...
      logger.error('Error in copyemoji:', error);
